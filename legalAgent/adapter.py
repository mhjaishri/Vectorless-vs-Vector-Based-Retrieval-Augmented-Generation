"""
Retrieval adapter for legalAgent (PageIndex RAG — Constitution + BNS + BNSS).
...
def _run(query: str) -> dict:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"legalAgent adapter failed:\nstdout: {result.stdout}\nstderr: {result.stderr}"
        )
    stdout = result.stdout.strip()
    json_start = stdout.rfind("{")
    if json_start == -1:
        raise RuntimeError(f"No JSON found in legalAgent output:\n{stdout}")
